	...
	public:
		...
		inline void printReport() const {
			...
			for (auto& p : timingData) {
				double avg = p.second / callCount.at(p.first);
				double pct = (totalTime > 0) ? (p.second / totalTime * 100.0) : 0.0;
		...
		inline double getTime(const char* name) const {
			auto it = timingData.find(name);
			return (it != timingData.end()) ? it->second : 0.0;
		}

	private:
		std::map<std::string, double> timingData;
